...
tracker_motorbike = Sort(max_age=20, min_hits=3,iou_threshold=0.3)

limits = [400,297,673,297]

total_count = []
#Nuevas contadores creados para las clases
car_count = []
motorbike_count = []
bus_count = []
truck_count = []
...
while True:
    ...
    detections_motorbike = np.empty((0,5))
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if currentClass == "car" :  
                ...
                current_array_two = np.array([x1,y1,x2,y2,conf])
                detections_motorbike = np.vstack((detections,current_array))
                
    results_tracker = tracker.update(detections)
    results_tracker_motorbike = tracker_motorbike.update(detections_motorbike)
    print("este es el result tracker",results_tracker)
    ...
    for result in results_tracker_motorbike:
        x1,y1,x2,y2,id = result
        x1,y1,x2,y2 = int(x1),int(y1),int(x2),int(y2)
        print(result)
        w, h = x2-x1,y2-y1
        cvzone.cornerRect(img,(x1,y1,w,h),l=10,rt=5,colorR=(255,56,56),colorC=(56,56,255))
        cvzone.putTextRect(img,f'{int(id)}',(max(0,x1),max(35,y1)),
                               scale=2,thickness=3,offset=10)
        ...
        #detectar el paso del objeto
        if limits[0] < cx < limits[2] and limits[1]-20 < cy < limits[1]+20:
            if motorbike_count.count(id) == 0:
                motorbike_count.append(id)
                cv2.line(img,(limits[0],limits[1]),(limits[2],limits[3]),(0,0,255),5)
        
        # cvzone.putTextRect(img,f'Contador: {len(total_count)}',(50,50),
        #                        scale=2,thickness=3,offset=10)
    cv2.putText(img,str(len(motorbike_count)),(355,100),cv2.FONT_HERSHEY_PLAIN,5,(56,56,255),8)
